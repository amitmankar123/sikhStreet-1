...
export const useCampaignStore = create((set, get) => ({
  ...
  updateCampaign: async (id, campaignData) => {
    ...
    try {
      ...
      set(state => ({
        campaigns: state.campaigns.map(c => (c.id || c._id) === id ? response.data : c),
        isLoading: false
      }));
  ...
  deleteCampaign: async (id) => {
    ...
    try {
      ...
      set(state => ({
        campaigns: state.campaigns.filter(c => (c.id || c._id) !== id),
        isLoading: false
      }));
  ...
  toggleCampaignStatus: async (id) => {
    const campaign = get().campaigns.find(c => (c.id || c._id) === id);
    if (!campaign) return;
