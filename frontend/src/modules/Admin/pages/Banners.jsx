...
const Banners = () => {
  ...
  const handleMoveUp = async (banner) => {
    const index = filteredBanners.findIndex((b) => (b.id || b._id) === (banner.id || banner._id));
    if (index <= 0) return;
    ...
    try {
      await reorderBannersApi([
        { id: current.id || current._id, order: previousOrder },
        { id: previous.id || previous._id, order: currentOrder },
      ]);
      await fetchBanners();
    } catch {
      // Store handles toast
  ...
  const handleMoveDown = async (banner) => {
    ...
    try {
      await reorderBannersApi([
        { id: current.id || current._id, order: nextOrder },
        { id: next.id || next._id, order: currentOrder },
      ]);
      await fetchBanners();
    } catch {
      // Store handles toast
          ...
          <>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {paginatedBanners.map((banner, index) => {
                ...
                const filteredIndex = filteredBanners.findIndex((b) => (b.id || b._id) === (banner.id || banner._id));
                return (
                  <div
