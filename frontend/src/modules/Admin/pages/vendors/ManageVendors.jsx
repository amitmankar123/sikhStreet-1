...
const ManageVendors = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      sortable: true,
      render: (value) => (
        <Badge variant={String(value || "Individual").toLowerCase()}>
          {value || "Individual"}
        </Badge>
      ),
    },
        ...
        <div className="flex items-center gap-2">
          ...
          {(row.status === "pending" || row.status === "suspended" || row.status === "rejected") && (
            <button
              ...
              title={row.status === "suspended" ? "Reactivate Vendor" : "Approve Vendor"}>
              <FiCheckCircle />
            </button>
          )}
          {row.status === "approved" && (
  ...
  const getModalContent = () => {
    switch (actionModal.type) {
      case "approve": {
        const v = vendors.find((vendor) => vendor.id === actionModal.vendorId);
        const isSuspended = v?.status === "suspended";
        return {
          title: isSuspended ? "Reactivate Vendor?" : "Approve Vendor?",
          message: isSuspended
          ...
          confirmText: isSuspended ? "Reactivate" : "Approve",
