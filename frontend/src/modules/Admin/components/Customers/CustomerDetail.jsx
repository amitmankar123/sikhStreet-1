...
import { FiX, FiMail, FiPhone, FiMapPin, FiShoppingBag, FiDollarSign, FiClock, FiTrash2, FiCreditCard } from 'react-icons/fi';
import { motion, AnimatePresence } from 'framer-motion';
import { useCustomerStore } from '../../../../shared/store/customerStore';
import Badge from '../../../../shared/components/Badge';
import { formatCurrency, formatDateTime } from '../../utils/adminHelpers';

const CustomerDetail = ({ customer: initialCustomer, onClose, onUpdate, startEditing = false }) => {
  const navigate = useNavigate();
  const location = useLocation();
  const isAppRoute = location.pathname.startsWith('/app');
  const { toggleCustomerStatus, deleteCustomer, addActivity, customers } = useCustomerStore();
  const [isSubmitting, setIsSubmitting] = useState(false);

  const customer = customers.find(c => String(c.id) === String(initialCustomer.id)) || initialCustomer;

  const handleDelete = async () => {
    if (window.confirm(`Are you sure you want to delete customer ${customer.name}? This will permanently remove all their addresses, reviews, and wishlist items.`)) {
