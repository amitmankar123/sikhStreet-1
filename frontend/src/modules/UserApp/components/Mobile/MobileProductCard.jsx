...
const MobileProductCard = ({ product }) => {
  ...
  return (
    <>
      <motion.div
        whileTap={{ scale: 0.98 }}
        className="bg-white border border-gray-100 rounded-2xl overflow-hidden mb-4 shadow-sm w-full"
        {...longPressHandlers}>
        <div className="flex gap-4 p-4">
          {/* Product Image */}
          <Link to={`/product/${product.id}`} className="w-24 h-24 flex-shrink-0 rounded-xl overflow-hidden bg-gray-50 block">
            <LazyImage
              src={product.image}
              alt={product.name}
