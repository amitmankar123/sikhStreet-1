          ...
          <div className="px-4 py-4 bg-white border-b border-gray-200 sticky top-0 z-30">
            <div className="flex items-center gap-3 mb-3">
              ...
              <div className="flex-1">
                <h1 className="text-2xl font-black text-slate-800 uppercase tracking-tight">Flash Sale</h1>
                <div className="relative mt-2">
                  ...
                  <input
                    ...
                    className="w-full pl-10 pr-10 py-2.5 bg-white border-2 border-gray-300 rounded-2xl text-sm font-semibold focus:outline-none focus:border-brand-navy focus:ring-1 focus:ring-brand-navy transition-all placeholder:text-gray-400 shadow-sm"
                  />
                  {searchQuery && (
                    <button onClick={() => setSearchQuery("")} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 p-1 hover:bg-gray-200 rounded-full">
              ...
              <div className="flex flex-row items-center gap-3">
                <div className="flex items-center bg-gray-100 rounded-xl p-1.5 h-11 shadow-sm border border-slate-200">
                  <button onClick={() => setViewMode("list")} className={`p-2 rounded-lg transition-colors ${viewMode === "list" ? "bg-white text-primary-600 shadow-sm" : "text-gray-600"}`}>
                    <FiList className="text-xl" />
                  </button>
                  <button onClick={() => setViewMode("grid")} className={`p-2 rounded-lg transition-colors ${viewMode === "grid" ? "bg-white text-primary-600 shadow-sm" : "text-gray-600"}`}>
                    <FiGrid className="text-xl" />
                  </button>
                </div>
                <div ref={filterButtonRef} className="relative">
                  <button onClick={() => setShowFilters(!showFilters)} className={`p-3 glass-card rounded-xl hover:bg-gray-100 transition-colors h-11 w-11 flex items-center justify-center shadow-sm border border-slate-200 ${showFilters ? "bg-gray-100" : ""}`}>
                    <FiFilter className={`text-xl transition-colors ${hasActiveFilters ? "text-blue-600" : "text-gray-600"}`} />
                  </button>
                  <AnimatePresence>
                    {showFilters && (
                      <>
