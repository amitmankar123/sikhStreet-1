...
import { FiFilter, FiArrowLeft, FiGrid, FiList, FiX, FiCheckCircle, FiStar, FiShoppingBag, FiHeart, FiMessageSquare, FiChevronDown, FiMail, FiSliders, FiSearch, FiTag } from "react-icons/fi";
import { motion, AnimatePresence } from "framer-motion";
import MobileLayout from "../components/Layout/MobileLayout";
import ProductListItem from "../components/Mobile/ProductListItem";
import { getProductsByBrand, getBrandById, getCatalogProducts } from "../data/catalogData";
...
const enrichBrand = (rawBrand) => {
    if (!rawBrand) return null;
    const name = rawBrand.name || "Brand Store";
    return {
        ...rawBrand,
        storeName: name,
        storeLogo: rawBrand.logo,
        storeDescription: `Welcome to ${name}! We offer high quality authentic products made with focus on detail and design excellence.`,
        rating: 4.7,
        reviewCount: 142,
        totalSales: "1.5k",
        joinDate: "2024-02-15",
...
const Brand = () => {
    ...
    const brand = useMemo(() => {
        if (mockBrandsMap[brandId]) {
            return mockBrandsMap[brandId];
        }
        const raw = getBrandById(brandId) || remoteBrand;
        return enrichBrand(raw);
    ...
    const rawBrandProducts = useMemo(() => {
        ...
        if (brandId === "heritage-forge") {
            ...
            local = getProductsByBrand(brandId);
        }
    ...
    const getCategoryCount = (catName) => {
        if (catName === "All") return brandProducts.length;
        return brandProducts.filter(p => p.categoryName === catName).length;
    };
    ...
    useEffect(() => {
        ...
        const fetchBrandData = async () => {
            if (!brandId || brandId === "heritage-forge") {
                if (active) {
            ...
            try {
                const [brandsRes, productsRes] = await Promise.all([
                    api.get("/brands/all"),
                    api.get("/products", { params: { brand: brandId, page: 1, limit: 100 } }),
                ]);
                ...
                for (let page = 2; page <= totalPages; page += 1) {
                    const nextRes = await api.get("/products", {
                        params: { brand: brandId, page, limit: 100 },
                    });
            ...
            <MobileLayout showBottomNav={true} showCartBar={true}>
                <div className="w-full max-w-[1400px] mx-auto px-0 md:px-12 py-0 md:py-6 bg-white min-h-screen text-left">
                    ...
                    <div className="bg-white px-4 md:px-0 py-4 md:py-8 flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-gray-200 relative">
                        <div className="flex flex-row items-center gap-4 md:gap-6">
                            ...
                            <div className="w-20 h-20 md:w-24 md:h-24 rounded-2xl overflow-hidden border border-gray-200 shadow-sm bg-white flex-shrink-0 p-1 -mt-10 md:-mt-16 z-10">
                                <LazyImage
                                    src={brand.storeLogo || brand.logo}
                                    alt={brand.storeName || brand.name}
                                    className="w-full h-full object-contain rounded-xl"
                                    onError={(e) => {
                                        e.target.src = getPlaceholderImage(96, 96, (brand.storeName || brand.name).charAt(0));
                                    }}
                                />
                            </div>
                                ...
                                <div className="flex flex-col md:flex-row md:items-center gap-1.5 md:gap-3 mt-1.5">
                                    ...
                                    <div className="hidden md:flex items-center gap-3 text-xs text-gray-500 font-semibold before:content-['|'] before:text-gray-300 before:hidden md:before:inline">
                                        <span className="flex items-center gap-1 text-gray-900 font-bold">
                                            ...
                                            {brand.rating || "4.9"} ({brand.reviewCount || "290"})
                                        </span>
                                        <span className="text-gray-300">|</span>
                                        <span>{brand.totalSales || "3.8k"} sales</span>
                                        <span className="text-gray-300">|</span>
                                        <span>{brand.joinDate ? `${2026 - new Date(brand.joinDate).getFullYear() || 1} year` : "1 year"} on Etsy</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="md:hidden mx-4 my-4 bg-white border border-gray-200 rounded-2xl shadow-sm p-4 grid grid-cols-3 text-center divide-x divide-gray-100">
                        <div>
                            <p className="text-lg font-bold text-gray-900 font-sans">{brand.totalSales || "3.8k"}</p>
                        ...
                        <div>
                            ...
                            <p className="text-[10px] text-gray-500 font-bold tracking-wider uppercase mt-0.5 font-sans">({brand.reviewCount || "290"})</p>
                        </div>
                        <div>
                            <p className="text-lg font-bold text-gray-900 font-sans">
                                {brand.joinDate ? `${2026 - new Date(brand.joinDate).getFullYear() || 1} Year` : "1 year"}
                            </p>
                        ...
                        <div id="reviews-section" className="scroll-mt-48 border-t border-gray-150 pt-12">
                            ...
                            <div className="bg-white rounded-2xl border border-gray-150 p-5 md:p-6 shadow-sm max-w-3xl mx-auto">
                                <div className="flex flex-col md:flex-row items-center gap-8 pb-6 border-b border-gray-100 mb-6">
                                    <div className="text-center">
                                        <p className="text-4xl font-extrabold text-gray-900">{brand.rating || "4.8"}</p>
                                        <div className="flex justify-center text-yellow-500 my-1.5">
                                            {[...Array(5)].map((_, i) => (
                                                <FiStar key={i} className={`text-base ${i < Math.floor(brand.rating || 4.8) ? "fill-current text-yellow-500" : "text-gray-200"}`} />
                                            ))}
                                        </div>
                                        <p className="text-xs text-gray-505 font-semibold">Shop rating ({brand.reviewCount || "150"} reviews)</p>
                                    </div>
                                        ...
                                        {[5, 4, 3, 2, 1].map((stars) => {
                                            ...
                                            return (
                                                <div key={stars} className="flex items-center gap-3">
                                                    ...
                                                    <span className="text-xs text-gray-550 w-8 text-right font-medium">{pct}%</span>
                                                </div>
                                            );
                                        })}
                                    </div>
                            ...
                            <div className="bg-white rounded-2xl border border-gray-150 p-5 md:p-6 shadow-sm max-w-3xl mx-auto space-y-6 text-left">
                                <div>
                                    ...
                                    <p className="text-sm text-gray-600 leading-relaxed font-sans">
                                        {brand.storeDescription || "Welcome to our shop! We craft premium heritage Sikh products including sterling silver Kada, traditional turbans, and authentic Sikh accessories. Every piece is made with dedication, preserving age-old craftsmanship traditions while maintaining top-notch modern quality standards."}
                                    </p>
                                </div>
                        ...
                        <div id="policies-section" className="scroll-mt-48 border-t border-gray-150 pt-12">
                            <div className="bg-white rounded-2xl border border-gray-150 p-5 md:p-6 shadow-sm max-w-3xl mx-auto space-y-6 text-left">
                                ...
                                <div className="pt-6 border-t border-gray-100">
                                    <h3 className="text-lg font-bold text-gray-950 mb-3">Returns & Exchanges</h3>
                                    <div className="space-y-1.5 text-sm text-gray-600 font-sans">
                                ...
                                <div className="pt-6 border-t border-gray-100">
                                    <h3 className="text-lg font-bold text-gray-950 mb-3">Payment Methods</h3>
