...
const MobileOffers = () => {
  ...
  useEffect(() => {
    ...
    const loadLiveOffers = async () => {
      try {
        ...
        console.warn("Backend campaigns failed, using fallback:", err);
        const offers = catalogProducts.filter((product) => product.flashSale || getDiscountPercent(product) > 0);
        if (!cancelled) {
    ...
    const loadAvailableCoupons = async () => {
      try {
        ...
        console.warn("Backend coupons failed, using fallback:", err);
        const staticCoupons = [
          { _id: "1", code: "WELCOME50", type: "percentage", value: 50, minOrderValue: 500 },
          { _id: "2", code: "FASHION20", type: "percentage", value: 20, minOrderValue: 999 },
          { _id: "3", code: "FREESHIP", type: "free_shipping", value: 0, minOrderValue: 1500 },
        ];
        if (!cancelled) {
          setAvailableCoupons(staticCoupons);
        }
      }
    };
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (filters.category) {
      // Helper to check if a category is a descendant of the target category
      ...
      result = result.filter(
        (product) => isDescendant(String(product.categoryId || ""), String(filters.category))
      );
    }
    if (filters.minPrice) {
      result = result.filter(
