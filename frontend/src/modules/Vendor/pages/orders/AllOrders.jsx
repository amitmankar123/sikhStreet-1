...
const AllOrders = () => {
  ...
  // Get per-vendor total (subtotal + shipping + tax - discount) from vendorItems
  const getVendorTotal = (order) => {
    const vendorItem = order.vendorItems?.find(
      (vi) => vi.vendorId?.toString() === vendorId?.toString()
    );
    if (!vendorItem) return order.total ?? order.totalAmount ?? 0;
    const sub = vendorItem.subtotal ?? 0;
    const ship = vendorItem.shipping ?? 0;
    const tax = vendorItem.tax ?? 0;
    const disc = vendorItem.discount ?? 0;
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_, row) => (
        <span className="font-semibold text-gray-800">
          {formatPrice(getVendorTotal(row))}
        </span>
      ),
    },
    {
            ...
            <div className="w-full sm:w-auto">
              <ExportButton
                ...
                headers={[
                  ...
                  { label: 'Amount', accessor: (row) => formatPrice(getVendorTotal(row)) },
