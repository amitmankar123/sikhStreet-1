...
const VendorLogin = ({ isModal = false, onClose }) => {
  ...
  const handleLoginSubmit = async (e) => {
    ...
    if (result?.success === false) {
      ...
      if (errMsg.toLowerCase().includes('verify your email') || errMsg.toLowerCase().includes('verify email')) {
        toast.error('Your email is not verified. Please enter the OTP sent to your email.');
        setIsOTPMode(true);
      } else if (
        status === 403 &&
        (errMsg.toLowerCase().includes('pending') || errMsg.toLowerCase().includes('approval'))
      ) {
        setIsPendingApproval(true);
      } else if (status === 403 && errMsg.toLowerCase().includes('suspended')) {
        toast.error(errMsg || 'Your account has been suspended. Contact support.');
      } else if (status === 403 && errMsg.toLowerCase().includes('rejected')) {
        toast.error(errMsg || 'Your vendor application was rejected.');
      } else {
  ...
  const handleOTPSubmit = async (e) => {
    ...
    try {
      ...
      if (loginResult?.success === false) {
        const errMsg = loginResult.message || '';
