...
/**
 * Check availability of email, phone, or storeName before registering
 * @param {{ email?, phone?, storeName? }} data
 */
export const checkVendorAvailability = (data) =>
    api.post('/vendor/auth/check-availability', data);
...
/**
 * Complete vendor post-approval onboarding
 */
export const completeVendorOnboarding = () =>
    api.patch('/vendor/auth/complete-onboarding');
...
/**
 * Upload a single vendor video using multer + Cloudinary pipeline
 * @param {File} file
 * @param {string} folder
 * @param {string} [publicId]
 */
export const uploadVendorVideo = (file, folder = 'vendors/products/videos', publicId) => {
    const formData = new FormData();
