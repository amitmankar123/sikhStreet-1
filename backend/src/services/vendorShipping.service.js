...
export const calculateVendorShippingForGroups = async ({
    ...
    const [zones, rates] = await Promise.all([
        VendorShippingZone.find({ vendorId: { $in: vendorIds } })
            .select('_id vendorId countries')
            .lean(),
        VendorShippingRate.find({ vendorId: { $in: vendorIds } })
            .select('_id vendorId zoneId name rate freeShippingThreshold')
            .lean(),
    ]);

    // Map _id to id so that the matching logic (e.g. rate.zoneId === zone.id) remains unbroken
    const mappedZones = zones.map((z) => ({ ...z, id: String(z._id) }));
    const mappedRates = rates.map((r) => ({ ...r, id: String(r._id) }));
    ...
    groups.forEach((group) => {
        ...
        const candidateRates = matchedZone
            ? vendorRates.filter((rate) => String(rate.zoneId) === String(matchedZone.id))
            : vendorRates;
