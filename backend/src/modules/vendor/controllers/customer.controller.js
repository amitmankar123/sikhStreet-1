...
const getCustomerIdFromOrder = (order) => {
    const userId = order?.user?._id || order?.user?.id || order?.userId;
...
const getVendorItemTotal = (order, vendorId) => {
    ...
    const vendorItem = vendorItemsArray.find(
...
export const getVendorCustomers = asyncHandler(async (req, res) => {
    ...
    const ordersWithUser = orders.map(o => {
        const user = users.find(u => String(u._id) === String(o.userId));
        return {
            ...o,
            user: user ? { ...user, id: String(user._id) } : null
        };
    });
...
export const getVendorCustomerById = asyncHandler(async (req, res) => {
    ...
    const customerOrders = ordersWithUser
    ...
    const detail = {
        ...
        lastOrderDate: firstOrder?.createdAt ?? null,
        orderHistory: paginatedHistory.map(o => ({ ...o, id: String(o._id) })),
        pagination: {
