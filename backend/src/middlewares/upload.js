...
const imageDiskStorage = multer.diskStorage({
    ...
    filename: (req, file, cb) => {
        ...
        cb(null, `${Date.now()}-${uniqueSuffix}-${safeBaseName}${ext}`);
    }
});
...
const videoFileFilter = (req, file, cb) => {
    const ext = path.extname(file.originalname || '').toLowerCase();
    const isValidExt = ['.mp4', '.webm', '.mov', '.mkv'].includes(ext);

    if (ALLOWED_VIDEO_MIME_TYPES.includes(file.mimetype) || (file.mimetype.startsWith('video/') || isValidExt)) {
        cb(null, true);
    } else {
...
// Single video upload
export const uploadVideo = (fieldName) =>
    multer({ storage: imageDiskStorage, fileFilter: videoFileFilter, limits: { fileSize: MAX_VIDEO_SIZE } }).single(fieldName);
...
// Multiple named document uploads (using TMP_UPLOAD_DIR for cloudinary upload)
export const uploadMultipleDocumentFields = (fields) =>
    multer({
        storage: imageDiskStorage,
        fileFilter: (req, file, cb) => {
            if (ALLOWED_DOCUMENT_MIME_TYPES.includes(file.mimetype)) {
