...
export const enforceAccountStatus = async (req, res, next) => {
    try {
        ...
        if (role === 'customer') {
            ...
            const user = await User.findOne({ _id: req.user.id }).select('isActive isVerified').lean();
        ...
        if (role === 'vendor') {
            ...
            const vendor = await Vendor.findOne({ _id: req.user.id }).select('status isVerified').lean();
        ...
        if (role === 'delivery') {
            ...
            const deliveryBoy = await DeliveryBoy.findOne({ _id: req.user.id }).select('applicationStatus isActive').lean();
        ...
        if (role === 'admin' || role === 'superadmin') {
            ...
            const admin = await Admin.findOne({ _id: req.user.id }).select('isActive').lean();
